# TEMPLATE – Manage metadata templates

# Name

iaarc  `template`<options>
...
--repo=[ master | derivative] or [ m | d ]

This selects the repository to be used. For example 

 `iavault template` --repo=master

Leaving out the --repo option will default to --repo=master

Example:

 `iavault template`--level=all 

This will list all the properties in both base and current.
...
`iavault template` --repo=master --level=base
